...
def _write_schema(cls: type[BaseModel], output_path: Path) -> None:
    cls.save_schema(str(output_path))
...
def _frozen_versions() -> dict:
    ...
    for family, versions in frozen.items():
        for version in versions:
            schema = MODELS_DIR / family / version / "schema.json"
            if not schema.is_file():
                raise SystemExit(
                    f"frozen.json lists {family} {version}, but "
                    f"{schema.relative_to(REPO_ROOT)} does not exist. A frozen version is "
                    "still served to consumers, so its schema.json must stay on disk."
                )
...
def generate() -> None:
    ...
    for family, versions in _frozen_versions().items():
        combined = set(available.get(family, [])) | set(versions)
        available[family] = sorted(combined, key=_version_sort_key)
        print(f"  {family}: {len(versions)} frozen version(s) kept in available.json")
