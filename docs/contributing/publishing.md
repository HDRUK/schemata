...
!!! note "Docs are not regenerated in CI"
    The markdown files in `docs/` are committed to the repository. CI does not regenerate them — it only checks they are current, and fails if they are not. Always run `python -m hdr_schemata.build` locally and commit the output before merging (see [Releasing a version](releasing.md)).
