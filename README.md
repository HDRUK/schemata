...
`frozen.json` lists versions that are no longer built from Pydantic models but are still
published. Their `schema.json` stays on disk exactly as last generated, and the build
merges them back into `available.json` so consumers can still resolve them by name and
version.

This exists because `available.json` is the schema registry traser compiles from: a
version that disappears from it stops being resolvable, and any stored record stamped
with that version fails validation. Freezing retires a version from maintenance without
withdrawing it from consumers. The build fails if a frozen version's `schema.json` is
missing.
